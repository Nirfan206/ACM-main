...
function EmployeeJobs() {
  ...
  useEffect(() => {
    // Initial fetch only; no intervals or automatic reloads
    fetchJobs();
  }, []);
  ...
  const handleStatusChange = async (jobId) => {
    ...
    try {
      ...
      const updatedJobs = jobs.map((job) =>
        job._id === jobId
          ? { ...job, status: statusUpdate, notes: notesUpdate.trim() }
          : job
      );
  ...
  return (
    <div className="card" style={{ maxWidth: 800, margin: "2rem auto" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <h3 style={{ marginBottom: "1.5rem", color: 'var(--color-text)' }}>Assigned Jobs</h3>
        <button onClick={fetchJobs} className="btn btn-outline">Refresh</button>
      </div>
            ...
            {jobs.length === 0 ? (
              ...
              jobs.map((job) => (
                <tr key={job._id}>
                  ...
                  <td>
                    <span
                      className={`status-badge status-${(job.status || 'Pending')
                        .toLowerCase()
                        .replace(' ', '-')
                  ...
                  <td>
                    <button
                      onClick={() => {
                        setSelectedJob(job);
                        setStatusUpdate(
                          job.status === 'Completed - Awaiting Admin Confirmation' ? 'Completed' : (job.status || '')
                        );
      ...
      {selectedJob && (
        <div className="card" style={{ marginTop: "20px" }}>
          ...
          <p><strong>Customer:</strong> {selectedJob.user?.profile?.name || 'N/A'}</p>
          <p>
            <strong>Phone:</strong>{" "}
            <a href={`tel:${selectedJob.user?.phone}`} style={{ textDecoration: 'none', color: 'var(--color-secondary)' }}>
              {selectedJob.user?.phone || 'N/A'}
            </a>
          </p>
          <p><strong>Address:</strong> {selectedJob.user?.profile?.address || 'N/A'}</p>
          <p style={{ marginBottom: "1rem" }}><strong>Service:</strong> {selectedJob.service?.type || 'N/A'}</p>
          ...
          <div className="form-actions-group">
            <button
              ...
              disabled={
                !statusUpdate ||
                (statusUpdate === selectedJob.status && notesUpdate === (selectedJob.notes || ""))
              }
              className="btn btn-success"
            >
              Update
            </button>
            <button onClick={() => setSelectedJob(null)} className="btn btn-danger">
              Cancel
            </button>
          </div>
        </div>
